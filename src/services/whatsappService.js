...
class WhatsAppService {
	...
	formatPhoneNumber(phoneNumber) {
		...
		const countryCode = cleanNumber.startsWith("00")
			? cleanNumber.substring(2)
			: cleanNumber.startsWith(DEFAULT_COUNTRY_CODE)
			? cleanNumber
			: `${DEFAULT_COUNTRY_CODE}${cleanNumber}`;

		return `${countryCode}${WHATSAPP_SUFFIX}`;
	}
